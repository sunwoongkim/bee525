...
void read_img (unsigned char *o_buf) {
  ...
  // convert the image format from RGB to Y (monochrome)
  for (int r=0; r<28; r++) {
    for (int c=0; c<28; c++) {
...
int main(void) {
  ...
  vector<double> input0 [28];
  Plaintext p0 [28];
  for (size_t j=0; j<28; j++) { // for 28 plaintext polynomials
    input0[j].reserve(slot_count);
    for (size_t i=0; i<slot_count; i++) {
      double norm_pxl = 0.0; 
      if (i < 28) { // use 28 slots out of 4K slots for 28 pixels on each row of the image
        norm_pxl = (double)plaintext_buf[28*j+i] / 255; 
      }
      input0[j].push_back(norm_pxl); // put 28 pixels into the input0[j] vector
    }
    encoder_client1.encode(input0[j], scale, p0[j]); // encode each vector
  }
  ...
  // encrypt the 28+1 plaintext polynomials
  Encryptor encryptor(context_client1, pk_client1);
  for (size_t j=0; j<28; j++) {
    auto size_c0 = encryptor.encrypt(p0[j]).save(of_cipher_client);
  }
